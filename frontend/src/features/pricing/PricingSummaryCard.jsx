    ...
    <div className="space-y-4">
      <aside className="rounded-2xl border border-[#FFEADA] bg-[#FFF8F4] p-5 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-[#F25C22]">{t('pricing.priceCalculator')}</h3>
        </div>

        <div className="rounded-xl border border-[#FFE8D6] bg-white p-4 space-y-1.5 text-xs text-[#A3AED0]">
          <p className="font-semibold text-[#1B2559]">{t('pricing.exampleOrder')}</p>
          <div className="flex justify-between">
            <span>{t('pricing.orderValue')}</span>
        ...
        <div className="rounded-xl border border-[#FFE8D6] bg-white p-4 space-y-1 text-xs text-[#A3AED0]">
          <p>{t('pricing.deliveryFee')}</p>
          <p className="text-[10px]">{t('pricing.deliveryFeeCalc', { base: `(${base})`, dist: DISTANCE, perKm })}</p>
          <p className="text-2xl font-extrabold text-[#1B2559]">ETB {deliveryFee}</p>
        </div>

        <div className="rounded-xl border border-[#FFE8D6] bg-white p-4 space-y-1 text-xs text-[#A3AED0]">
          <p>{t('pricing.serviceCharge')}</p>
          <p className="text-[10px]">{t('pricing.serviceChargeCalc', { value: ORDER_VALUE, pct: servicePct })}</p>
        ...
        <div className="flex items-center justify-between rounded-xl bg-[#F25C22] px-5 py-4 text-white">
          <div>
            <p className="text-xs font-semibold opacity-80">{t('pricing.totalFees')}</p>
      ...
      <aside className="rounded-2xl border border-[#E0E5F2] bg-white p-5 shadow-sm space-y-4">
        <h3 className="text-sm font-bold text-[#1B2559]">{t('pricing.quickStats')}</h3>
        <div className="space-y-3">
          <div>
            <p className="text-xs text-[#A3AED0]">{t('pricing.avgDeliveryFee')}</p>
            <p className="text-xl font-extrabold text-[#1B2559]">ETB 85</p>
          </div>
          <div>
            <p className="text-xs text-[#A3AED0]">{t('pricing.avgDistance')}</p>
            <p className="text-xl font-extrabold text-[#1B2559]">3.2 km</p>
          </div>
          <div>
            <p className="text-xs text-[#A3AED0]">{t('pricing.totalRevenue')}</p>
            <p className="text-xl font-extrabold text-[#1B2559]">ETB 15,240</p>
            <p className="text-xs text-[#A3AED0]">{t('pricing.thisMonth')}</p>
          </div>
        </div>
      </aside>
    </div>
