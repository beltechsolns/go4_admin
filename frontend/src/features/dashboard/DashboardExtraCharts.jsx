...
const TW = 400, TH = 200, TPL = 40, TPR = 10, TPT = 10, TPB = 30
const TCW = TW - TPL - TPR, TCH = TH - TPT - TPB

const PW = 400, PH = 200, PPL = 38, PPR = 10, PPT = 10, PPB = 30
const PCW = PW - PPL - PPR, PCH = PH - PPT - PPB

const PIE_R = 70, PIE_CX = 120, PIE_CY = 100

function Skeleton() {
  return <div className="h-52 rounded-xl bg-[#F4F7FE] animate-pulse" />
}
...
function DeliveryTrendsChart({ data, loading }) {
  ...
  const tX = (i) => TPL + (i / (Math.max(data.length - 1, 1))) * TCW
  const buildLine = (key) => data.map((d, i) => `${i === 0 ? 'M' : 'L'} ${tX(i)} ${tY(d[key])}`).join(' ')
...
function PeakHoursChart({ data, loading }) {
  ...
  const pBarW = (PCW / Math.max(data.length, 1)) * 0.6
  const pBarGap = PCW / Math.max(data.length, 1)
  ...
  return (
    <svg viewBox={`0 0 ${PW} ${PH}`} className="w-full h-auto">
      ...
      {data.map((h, i) => {
        ...
        const showLabel = data.length <= 8 || i % 3 === 0
        return (
          <g key={i}>
...
function OrdersByCategoryChart({ data, loading }) {
  ...
  return (
    <div className="flex items-center gap-4">
      <svg viewBox="0 0 260 200" className="w-48 h-auto shrink-0">
        {slices.map((s, i) => (
          <g key={i}>
