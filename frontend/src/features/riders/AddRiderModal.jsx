...
export default function AddRiderModal({ onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.full_name.trim() || !form.phone.trim()) {
      setError(t('riders.namePhoneRequired'))
      return
    }
    setSaving(true)
    setError('')
    try {
      ...
      setError(err.response?.data?.error || t('riders.failedToAdd'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Modal title={t('riders.addRider')} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        {error && <p className="rounded-xl bg-red-50 px-3 py-2 text-sm text-red-500">{error}</p>}

        <div>
          <label className="mb-1 block text-xs font-semibold text-[#1B2559]">{t('riders.fullName')} *</label>
        ...
        <div>
          <label className="mb-1 block text-xs font-semibold text-[#1B2559]">{t('riders.phone')} *</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-xs font-semibold text-[#1B2559]">{t('riders.vehicle')}</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-[#1B2559]">{t('riders.status')}</label>
        ...
        <div>
          <label className="mb-1 block text-xs font-semibold text-[#1B2559]">{t('riders.zone')}</label>
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <button type="submit" disabled={saving}
            ...
            {saving ? t('riders.adding') : t('riders.addRider')}
          </button>
        </div>
      </form>
    </Modal>
