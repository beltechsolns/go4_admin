...
export default function RiderPerformanceTable() {
  ...
  return (
    <Card title={t('reports.riderPerformanceDetails')}>
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-[#F4F7FE]">
              <th className="pb-3 text-xs font-bold text-[#1B2559]">{t('reports.rider')}</th>
              <th className="pb-3 text-xs font-bold text-[#1B2559]">{t('reports.totalDeliveries')}</th>
              <th className="pb-3 text-xs font-bold text-[#1B2559]">{t('reports.successRate')}</th>
              <th className="pb-3 text-xs font-bold text-[#1B2559]">{t('reports.avgTime')}</th>
              <th className="pb-3 text-xs font-bold text-[#1B2559]">{t('reports.performance')}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#F4F7FE]">
            {loading ? (
              <tr><td colSpan={5} className="py-8 text-center text-sm text-[#A3AED0]">{t('reports.loading')}</td></tr>
            ) : performers.length === 0 ? (
              <tr><td colSpan={5} className="py-8 text-center text-sm text-[#A3AED0]">{t('reports.noPerformanceData')}</td></tr>
            ) : performers.map((item) => (
              <tr key={item.id} className="hover:bg-[#FAFAFA]">
                ...
                <td className="py-3.5 pr-4 text-[#1B2559]">{item.avg_delivery_time} {t('dashboard.minutes')}</td>
