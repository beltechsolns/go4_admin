...
export default function AddCustomerModal({ onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.full_name.trim() || !form.phone.trim()) {
      setError(t('customers.namePhoneRequired'))
      return
    }
    setSaving(true)
    setError('')
    try {
      ...
      setError(err.response?.data?.error || t('customers.failedToAdd'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Modal title={t('customers.addCustomer')} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        {error && <p className="rounded-xl bg-red-50 px-3 py-2 text-sm text-red-500">{error}</p>}

        <div>
          <label className="mb-1 block text-xs font-semibold text-[#1B2559]">{t('customers.fullName')} *</label>
        ...
        <div>
          <label className="mb-1 block text-xs font-semibold text-[#1B2559]">{t('customers.phone')} *</label>
        ...
        <div>
          <label className="mb-1 block text-xs font-semibold text-[#1B2559]">{t('customers.emailOptional')}</label>
        ...
        <div>
          <label className="mb-1 block text-xs font-semibold text-[#1B2559]">{t('customers.status')}</label>
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <button type="submit" disabled={saving}
            ...
            {saving ? t('customers.adding') : t('customers.addCustomer')}
          </button>
        </div>
      </form>
    </Modal>
